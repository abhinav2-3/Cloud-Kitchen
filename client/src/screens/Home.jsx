...
const Home = () => {
  ...
  const { items, categoryData, isLoading } = useProductContext();
          ...
          {categoryData.map((data, index) => {
            return (
              <div className="flex flex-col px-12" key={index}>
                <h1 className="text-4xl py-4 font-bold uppercase text-yellow-300">
                  {data.CategoryName}
                </h1>
                <div className="flex gap-8 flex-wrap w-full justify-start">
                  {items
                    .filter(
                      (fooditems) =>
